...
net = cv2.dnn.readNetFromONNX('yolov5/best.onnx')
...
def log_status(angle, distance=None):
    ...
    if distance is not None:
        global instance_id,turnNumber
        # TEMP image path
        temp_img_name = f"{turnNumber}_{angle}deg_temp.jpg"
        temp_img_path = os.path.join(instance_id, temp_img_name)
        camera.capture_file(temp_img_path)
        ...
        # Determine pollutant info
        class_names = [labels[i] for i in class_ids]
        ...
        # Rename and move temp image
        final_path = os.path.join(instance_id, img_name)
